...
export class SupportingDocumentsController {
  ...
  @Get()
  list(@Req() req: { user: RequestUser }, @Query('invoiceId') invoiceId?: string) {
...
export class PaymentBatchesController {
  ...
  @Post('from-approved')
  @Roles(Role.COMPANY_ADMIN, Role.AP_CLERK)
  createFromApproved(@Body() body: BodyRecord, @Req() req: { user: RequestUser }) {
  ...
  @Post('bank-response')
  @Roles(Role.COMPANY_ADMIN, Role.AP_CLERK)
  importBankResponse(@Body() body: BodyRecord, @Req() req: { user: RequestUser }) {
