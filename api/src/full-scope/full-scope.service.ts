...
function optionalNumber(body: Body, key: string, fallback = 0) {
  ...
  if (!Number.isFinite(parsed)) throw new BadRequestException(`${key} must be numeric`);
  return parsed;
}
...
function requiredDate(body: Body, key: string) {
  ...
  if (Number.isNaN(date.getTime())) throw new BadRequestException(`${key} must be a date`);
  return date;
}
...
export class FullScopeService {
  ...
  async overview(user: RequestUser) {
    ...
    const [
      ...
      this.prisma.paymentRecord.count({
        where: {
          status: { in: [PaymentRecordStatus.SCHEDULED, PaymentRecordStatus.PENDING] },
  ...
  async createPurchaseOrder(body: Body, user: RequestUser) {
    if (user.role === Role.AP_CLERK) {
      throw new ForbiddenException('AP can verify POs but department/company creates them');
    }
    await this.assertDepartmentAccess(requiredText(body, 'departmentId'), user);
    const lineItems = Array.isArray(body.lineItems) ? (body.lineItems as Body[]) : [];
    const subtotal = lineItems.reduce(
      (sum, line) =>
        sum +
        optionalNumber(line, 'quantity', 1) * optionalNumber(line, 'unitPrice', 0),
      0,
    );
    ...
    await this.audit('purchase_order', po.id, AuditAction.CREATE, user, undefined, po);
    return po;
  }

  async addSupportingDocument(body: Body, user: RequestUser) {
    if (text(body, 'invoiceId')) await this.assertInvoiceAccess(requiredText(body, 'invoiceId'), user);
    if (text(body, 'poId')) await this.assertPurchaseOrderAccess(requiredText(body, 'poId'), user);
    const doc = await this.prisma.supportingDocument.create({
      data: {
        ...
        purchaseOrder: text(body, 'poId')
          ...
          : undefined,
        documentType: enumValue(DocumentType, body, 'documentType', DocumentType.OTHER),
    ...
    await this.audit('supporting_document', doc.id, AuditAction.CREATE, user, undefined, doc);
    return doc;
  }
  ...
  async createVerification(body: Body, user: RequestUser) {
    ...
    const verification = await this.prisma.verification.create({
      data: {
        ...
        status: enumValue(VerificationStatus, body, 'status', VerificationStatus.PENDING),
        comments: text(body, 'comments'),
        verifiedAt:
          text(body, 'status') && text(body, 'status') !== VerificationStatus.PENDING
            ? new Date()
            : undefined,
      },
    });
    await this.audit('verification', verification.id, AuditAction.CREATE, user, undefined, verification);
    return verification;
  }
  ...
  async listVerifications(user: RequestUser, invoiceId?: string) {
    return this.prisma.verification.findMany({
      ...
      include: { verifiedBy: { select: { id: true, name: true, email: true } }, invoice: true },
      orderBy: { id: 'desc' },
    });
  }
  ...
  async raiseQuery(body: Body, user: RequestUser) {
    ...
    await this.audit('interdepartmental_query', query.id, AuditAction.CREATE, user, undefined, query);
  ...
  async respondQuery(id: string, body: Body, user: RequestUser) {
    ...
    await this.audit('interdepartmental_query', id, AuditAction.UPDATE, user, undefined, query);
    return query;
  }
  ...
  async closeQuery(id: string, user: RequestUser) {
    ...
    await this.audit('interdepartmental_query', id, AuditAction.STATUS_CHANGE, user, undefined, query);
    return query;
  }
  ...
  async createApprovalMatrixRule(body: Body, user: RequestUser) {
    ...
    await this.audit('approval_matrix', rule.id, AuditAction.CREATE, user, undefined, rule);
    return rule;
  }
  ...
  async listApprovalMatrix(user: RequestUser) {
    ...
    return this.prisma.approvalMatrix.findMany({
      include: { department: true, requiredUser: { select: { id: true, name: true, email: true } } },
  ...
  async createBatchFromApproved(body: Body, user: RequestUser) {
    ...
    const invoiceIds = Array.isArray(body.invoiceIds)
      ? (body.invoiceIds.filter((id) => typeof id === 'string') as string[])
      : [];
    ...
    if (!invoices.length) throw new BadRequestException('No approved invoices available');
    const eligible = invoices.filter((invoice) => invoice.vendorId && invoice.vendor);
    if (!eligible.length) throw new BadRequestException('Approved invoices need linked vendors');
    const total = eligible.reduce((sum, invoice) => sum + Number(invoice.balanceDue || invoice.amountPkr), 0);
    const batch = await this.prisma.paymentBatch.create({
      data: {
        batchNumber: `PB-${Date.now()}`,
        batchDate: body.batchDate ? requiredDate(body, 'batchDate') : new Date(),
        currency: text(body, 'currency') ?? 'PKR',
        ...
        payments: {
          create: eligible.map((invoice, index) => ({
            ...
            paymentDate: body.paymentDate ? requiredDate(body, 'paymentDate') : new Date(),
            paymentMethod: enumValue(PaymentMethod, body, 'paymentMethod', PaymentMethod.BANK_PORTAL),
            amount: invoice.balanceDue.gt(0) ? invoice.balanceDue : invoice.amountPkr,
    ...
    await this.audit('payment_batch', batch.id, AuditAction.CREATE, user, undefined, batch);
    return batch;
  }
  ...
  async exportMeezanBatch(id: string, user: RequestUser) {
    ...
    await this.audit('payment_batch', id, AuditAction.EXPORT, user, undefined, { csvRows: rows.length });
    return {
  ...
  async importBankResponse(body: Body, user: RequestUser) {
    ...
    await this.audit('payment_response', 'bank-response', AuditAction.IMPORT, user, undefined, results);
  ...
  async reconcilePayment(body: Body, user: RequestUser) {
    ...
    const reconciliation = await this.prisma.reconciliation.create({
      data: {
        ...
        statementDate: body.statementDate ? requiredDate(body, 'statementDate') : new Date(),
        reconciledAmount: decimal(optionalNumber(body, 'reconciledAmount', 0)),
        status: enumValue(ReconciliationStatus, body, 'status', ReconciliationStatus.MATCHED),
        discrepancyNotes: text(body, 'discrepancyNotes'),
        reconciledBy: { connect: { id: user.id } },
      },
    });
    await this.audit('reconciliation', reconciliation.id, AuditAction.CREATE, user, undefined, reconciliation);
  ...
  async listReconciliations(user: RequestUser) {
    ...
    return this.prisma.reconciliation.findMany({
      include: { payment: { include: { vendor: true, invoice: true } }, reconciledBy: true },
  ...
  xeroAuthUrl(state = 'company-ap') {
    ...
    if (!clientId || !redirectUri) {
      throw new BadRequestException('XERO_CLIENT_ID and XERO_REDIRECT_URI are required');
    }
  ...
  async xeroCallback(code: string) {
    ...
    if (!tokenRes.ok) {
      throw new BadRequestException(`Xero token exchange failed: ${await tokenRes.text()}`);
    }
    ...
    if (!connectionsRes.ok) {
      throw new BadRequestException(`Xero connections failed: ${await connectionsRes.text()}`);
    }
  ...
  async syncTicketToXero(ticketId: string, user: RequestUser) {
    ...
    if (ticket.status !== TicketStatus.XERO_BILL_ENTRY) {
      throw new BadRequestException('Xero bill can only be created at the Xero bill entry step');
    }
        ...
        Invoices: [
          {
            ...
            Contact: ticket.xeroContactId
              ...
              : { Name: ticket.vendor?.displayName ?? ticket.vendorNameSnapshot ?? 'Unknown vendor' },
    ...
    if (!response.ok) {
      await this.prisma.paymentTicket.update({
        where: { id: ticketId },
        data: { xeroSyncStatus: XeroSyncStatus.SYNC_FAILED, xeroError: JSON.stringify(result) },
      });
      throw new BadRequestException(`Xero sync failed`);
    }
    const xeroBillId = (result as { Invoices?: Array<{ InvoiceID?: string; InvoiceNumber?: string }> })
      .Invoices?.[0]?.InvoiceID;
    const xeroBillNumber = (result as { Invoices?: Array<{ InvoiceID?: string; InvoiceNumber?: string }> })
    ...
    await this.audit('payment_ticket', ticketId, AuditAction.SYNC, user, undefined, updated);
  ...
  async markTicketPaidInXero(ticketId: string, body: Body, user: RequestUser) {
    ...
    if (ticket.status !== TicketStatus.BANK_EXECUTED) {
      throw new BadRequestException('Payment can only be marked paid after bank execution');
    }
    if (!ticket.xeroBillId) {
      throw new BadRequestException('Create the Xero bill before marking it paid');
    }
    const connection = await this.activeXeroConnection();
    const accountCode =
      text(body, 'accountCode') ?? this.config.get<string>('XERO_BANK_ACCOUNT_CODE');
    if (!accountCode) {
      throw new BadRequestException('XERO_BANK_ACCOUNT_CODE or accountCode is required');
    }
        ...
        Payments: [
          {
            ...
            Amount: amount,
            Reference: text(body, 'reference') ?? ticket.bankPortalReference ?? ticket.voucherNumber,
          },
        ],
      }),
    });
    ...
    const xeroPaymentId = (result as { Payments?: Array<{ PaymentID?: string }> })
  ...
  private departmentScopedWhere(user: RequestUser): Prisma.PurchaseOrderWhereInput {
    if (user.role === Role.DEPT_USER || user.role === Role.DEPT_ADMIN) {
      return { departmentId: user.departmentId ?? '__no_department__' };
    }
    return {};
  ...
  private queryScopeWhere(user: RequestUser): Prisma.InterdepartmentalQueryWhereInput {
    if (user.role !== Role.DEPT_USER && user.role !== Role.DEPT_ADMIN) return {};
  ...
  private async assertDepartmentAccess(departmentId: string, user: RequestUser) {
    if (
      (user.role === Role.DEPT_USER || user.role === Role.DEPT_ADMIN) &&
      user.departmentId !== departmentId
    ) {
      throw new ForbiddenException('Department users can only operate on their own department');
    }
  ...
  private async refreshXeroConnection(connection: {
    ...
    if (!clientId || !clientSecret || !connection.refreshToken) {
      throw new BadRequestException('Xero refresh token or client credentials are missing');
    }
    const body = new URLSearchParams({
    ...
    if (!tokenRes.ok) {
      throw new BadRequestException(`Xero token refresh failed: ${await tokenRes.text()}`);
    }
    const token = (await tokenRes.json()) as {
      access_token: string;
      refresh_token?: string;
    ...
    if (!updated.accessToken) {
      throw new BadRequestException('Xero token refresh did not return an access token');
    }
