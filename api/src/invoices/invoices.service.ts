...
export class InvoicesService {
  ...
  private async upsertDepartmentTicketFromInvoice(
    invoiceId: string,
    ...
    const data = {
      ...
      notes:
        inv.description ??
        `Created from invoice upload: ${inv.originalFilename ?? invoiceId}`,
  ...
  private async createTicketFromInvoice(
    invoiceId: string,
    submittedById: string,
  ) {
    const inv = await this.prisma.invoice.findUnique({
      ...
      data: {
        ...
        activities: {
          create: {
            ...
            type: 'invoice_uploaded',
            message:
              'AP ticket created automatically from invoice upload/import',
            toStatus: needsVendorReview
              ? TicketStatus.VENDOR_PO_ACCOUNT_VERIFICATION
              : TicketStatus.DOCS_REVIEW,
          },
  ...
  private async ensureInvoicePurchaseOrder(
    invoiceId: string,
    ...
    const vendorId =
      inv.vendorId ??
      (await this.ensurePendingVendor(inv.departmentId, inv.department.name));
    ...
    const lineDescription =
      inv.description ?? inv.reference ?? inv.originalFilename ?? poNumber;
  ...
  private async ensurePendingVendor(
    departmentId: string,
  ...
  private async runAgentVerification(invoiceId: string) {
    ...
    const invoiceTotal = inv.totalAmount.gt(0)
      ? inv.totalAmount
      : inv.amountPkr;

    if (!inv.invoiceNumber && !inv.reference)
      errors.push('Invoice number or reference is required');
    ...
    const extracted =
      inv.extracted &&
      typeof inv.extracted === 'object' &&
      !Array.isArray(inv.extracted)
        ? inv.extracted
        : {};
    ...
    if (errors.length) {
      throw new BadRequestException(
        `Agent verification failed: ${errors.join('; ')}`,
      );
    }
  ...
  async releaseApprovedInvoiceToFinance(invoiceId: string, actorId: string) {
    ...
    if (inv.status !== InvoiceStatus.APPROVED) {
      throw new BadRequestException(
        'Invoice must be department-head approved before finance release',
      );
    }
    if (inv.poId) {
      await this.prisma.purchaseOrder.update({
  ...
  async returnRejectedInvoiceToDepartment(
    invoiceId: string,
    actorId: string,
    reason?: string,
  ) {
    await this.prisma.paymentTicket.updateMany({
      ...
      data: {
        ...
        notes: reason
          ? `Rejected by department head: ${reason}`
  ...
  private async applyVendorMatch(invoiceId: string) {
    ...
    const vendorTaxNumber =
      typeof e.vendorTaxNumber === 'string' ||
      typeof e.vendorTaxNumber === 'number'
        ? String(e.vendorTaxNumber).trim()
        : '';
    const vendorName =
      typeof e.vendorName === 'string' || typeof e.vendorName === 'number'
        ? String(e.vendorName).trim().toLowerCase()
        : '';

    if (vendorTaxNumber) {
      vendor = await this.prisma.vendor.findFirst({
        where: { taxNumber: vendorTaxNumber, active: true },
      });
    }
    if (!vendor && vendorName) {
      ...
      vendor =
        list.find(
          (v) =>
            v.displayName.toLowerCase().includes(vendorName) ||
            vendorName.includes(v.displayName.toLowerCase()),
        ) ?? null;
    }
  ...
  async patchInvoice(
    ...
    if (user.role === Role.DEPT_USER) {
      if (inv.departmentId !== user.departmentId) {
        throw new ForbiddenException(
          'Invoice is outside your department scope',
        );
      }
      if (
        !(
    ...
    if (user.role === Role.DEPT_ADMIN) {
      throw new ForbiddenException(
        'Department head can review, approve, or reject, not edit invoice details',
      );
    }
    if (
    ...
    if (dto.departmentId) {
      if (
        ...
        throw new ForbiddenException(
          'Department users cannot move invoices to another department',
        );
      }
    ...
    if (dto.taxAmount != null)
      data.taxAmount = new Prisma.Decimal(dto.taxAmount);
  ...
  private async syncTicketFromInvoice(invoiceId: string) {
    ...
    await this.prisma.paymentTicket.update({
      ...
      data: {
        ...
        vendor: inv.vendorId ? { connect: { id: inv.vendorId } } : undefined,
        vendorNameSnapshot:
          inv.vendor?.displayName ?? ticket.vendorNameSnapshot ?? null,
  ...
  async submitForApproval(
    ...
    if (
      user.role === Role.DEPT_USER &&
      inv.departmentId !== user.departmentId
    ) {
      throw new ForbiddenException('Invoice is outside your department scope');
    }
    if (user.role === Role.DEPT_ADMIN) {
      throw new ForbiddenException(
        'Department head cannot submit requests to themselves',
      );
    }
    ...
    await this.prisma.paymentTicket.updateMany({
      ...
      data: {
        ...
        notes:
          'Agent verification passed; waiting for department head approval',
      },
    });
    ...
    const inv = await this.prisma.invoice.create({
      data: {
        ...
        description:
          ...
          'Imported from published spreadsheet (CSV) URL',
        mimeType: 'text/csv',
  ...
  private assertDepartmentCanCreateInvoice(
    ...
    if (user.role === Role.CFO) {
      throw new ForbiddenException(
        'CFO can authorize payments, not create invoices',
      );
    }
    if (user.role === Role.DEPT_ADMIN) {
      throw new ForbiddenException(
        'Department head can approve or reject, not create invoices',
      );
    }
    if (user.role === Role.DEPT_USER && user.departmentId !== departmentId) {
      throw new ForbiddenException(
        'Department users can only create invoices for their own department',
      );
    }
  }
}
